...
if __name__ == '__main__':
    ...
    patch_file_url = str.format('%s/ftp/%s/%s_%s/%s' % (FILES_SERVER_URL, 'cpp', '3.4', '3.5', PATCH_FILE_NAME))
    # patch_file_url = str.format('%s/%s/%s_%s/%s' % (FILES_SERVER_URL, project_type, project_version, args.upgradeVersion, PATCH_FILE_NAME))
    patch_file_path = os.path.join(os.getcwd(), PATCH_FILE_NAME)
